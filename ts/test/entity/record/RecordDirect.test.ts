...
describe('RecordDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new NeugeborenenVornamenKantonStgallenSDK({
  ...
  test('direct-list-record', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-record', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['NEUGEBORENEN_VORNAMEN_KANTON_STGALLEN_TEST_RECORD_ENTID']
