...
local RecordEntity = {}
RecordEntity.__index = RecordEntity


function RecordEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RecordEntity)
  self._name = "record"
  self._client = client
...
function RecordEntity:get_name()
  return self._name
end


function RecordEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RecordEntity.new(self._client, opts)
end
...
function RecordEntity:mark_deleted()
  self._deleted = true
end


function RecordEntity:deleted()
  return true == self._deleted
end


function RecordEntity:data_set(args)
  if args ~= nil then
...
function RecordEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RecordEntity:match_set(args)
  if args ~= nil then
...
function RecordEntity:match_get()
...
function RecordEntity:stream(action, args, callopts)
...
function RecordEntity:_run_op(ctx, post_done)
  local utility = self._utility
