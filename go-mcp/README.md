# neugeborenen-vornamen-kanton-stgallen-mcp

[MCP](https://modelcontextprotocol.io) server exposing the NeugeborenenVornamenKantonStgallen SDK as
two agent tools — `neugeborenen-vornamen-kanton-stgallen_list` and `neugeborenen-vornamen-kanton-stgallen_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/neugeborenen-vornamen-kanton-stgallen-mcp)
make build

# 2. Provide credentials via the environment
export NEUGEBORENEN_VORNAMEN_KANTON_STGALLEN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user neugeborenen-vornamen-kanton-stgallen \
  -- /absolute/path/to/neugeborenen-vornamen-kanton-stgallen-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./neugeborenen-vornamen-kanton-stgallen-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// neugeborenen-vornamen-kanton-stgallen_list: first page of records
{ "entity": "metadata" }
{ "entity": "metadata", "query": { } }

// neugeborenen-vornamen-kanton-stgallen_load: one record by id
{ "entity": "metadata", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/neugeborenen-vornamen-kanton-stgallen-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user neugeborenen-vornamen-kanton-stgallen \
     -- "$PWD"/dist/*/neugeborenen-vornamen-kanton-stgallen-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `neugeborenen-vornamen-kanton-stgallen_list` and `neugeborenen-vornamen-kanton-stgallen_load` tools now appear
   in new sessions. Ask the agent to *"list metadata using neugeborenen-vornamen-kanton-stgallen"*
   and it calls `neugeborenen-vornamen-kanton-stgallen_list` with `{"entity":"metadata"}`.
...
```sh
export NEUGEBORENEN_VORNAMEN_KANTON_STGALLEN_APIKEY=sk_live_xxx            # API key
export NEUGEBORENEN_VORNAMEN_KANTON_STGALLEN_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `neugeborenen-vornamen-kanton-stgallen_list` tool
...
### Call the `neugeborenen-vornamen-kanton-stgallen_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `neugeborenen-vornamen-kanton-stgallen_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `neugeborenen-vornamen-kanton-stgallen_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NEUGEBORENEN_VORNAMEN_KANTON_STGALLEN_APIKEY` | API key sent with every request. |
| `NEUGEBORENEN_VORNAMEN_KANTON_STGALLEN_BASE` | Optional override of the API base URL. |
...
```sh
./neugeborenen-vornamen-kanton-stgallen-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"neugeborenen-vornamen-kanton-stgallen_load","arguments":{"entity":"metadata","query":{"id":1}}}}'
```
