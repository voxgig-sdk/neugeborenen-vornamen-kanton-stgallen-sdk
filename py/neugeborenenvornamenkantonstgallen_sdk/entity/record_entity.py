...
from neugeborenenvornamenkantonstgallen_sdk.neugeborenenvornamenkantonstgallen_types import (
    Record,
    RecordListMatch,
)
...
class RecordEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return RecordEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Record]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Record().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
