...
class TestRecordEntity:
    ...
    def test_should_stream(self):
        ...
        base = NeugeborenenVornamenKantonStgallenSDK.test(seed, None)
        seen = list(base.Record(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = NeugeborenenVornamenKantonStgallenSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Record(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set NEUGEBORENEN_VORNAMEN_KANTON_STGALLEN_TEST_RECORD_ENTID JSON to run live")
        client = setup["client"]
        ...
        record_ref01_list_result = record_ref01_ent.list(record_ref01_match, None)
...
def _record_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("NEUGEBORENEN_VORNAMEN_KANTON_STGALLEN_TEST_RECORD_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("NEUGEBORENEN_VORNAMEN_KANTON_STGALLEN_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = NeugeborenenVornamenKantonStgallenSDK(helpers.to_map(merged_opts))

    _live = env.get("NEUGEBORENEN_VORNAMEN_KANTON_STGALLEN_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("NEUGEBORENEN_VORNAMEN_KANTON_STGALLEN_TEST_EXPLAIN") == "TRUE",
        "live": _live,
