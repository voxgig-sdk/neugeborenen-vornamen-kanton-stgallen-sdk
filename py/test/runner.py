# NeugeborenenVornamenKantonStgallen SDK test runner
...
class NeugeborenenVornamenKantonStgallenTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NeugeborenenVornamenKantonStgallenTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NeugeborenenVornamenKantonStgallenTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NeugeborenenVornamenKantonStgallenTestRunner.getenv("NEUGEBORENEN_VORNAMEN_KANTON_STGALLEN_TEST_LIVE")
        override = NeugeborenenVornamenKantonStgallenTestRunner.getenv("NEUGEBORENEN_VORNAMEN_KANTON_STGALLEN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NeugeborenenVornamenKantonStgallenTestRunner.getenv(key)
        ...
        explain = NeugeborenenVornamenKantonStgallenTestRunner.getenv("NEUGEBORENEN_VORNAMEN_KANTON_STGALLEN_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["NEUGEBORENEN_VORNAMEN_KANTON_STGALLEN_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if NeugeborenenVornamenKantonStgallenTestRunner._test_control is not None:
            return NeugeborenenVornamenKantonStgallenTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NeugeborenenVornamenKantonStgallenTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NeugeborenenVornamenKantonStgallenTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NeugeborenenVornamenKantonStgallenTestRunner.load_test_control()
...
def load_env_local():
    NeugeborenenVornamenKantonStgallenTestRunner.load_env_local()


def env_override(m):
    return NeugeborenenVornamenKantonStgallenTestRunner.env_override(m)


def entity_data(v):
    return NeugeborenenVornamenKantonStgallenTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NeugeborenenVornamenKantonStgallenTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NeugeborenenVornamenKantonStgallenTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NeugeborenenVornamenKantonStgallenTestRunner.load_test_control()


def live_client_options():
    return NeugeborenenVornamenKantonStgallenTestRunner.live_client_options()


def live_delay_ms():
    return NeugeborenenVornamenKantonStgallenTestRunner.live_delay_ms()
