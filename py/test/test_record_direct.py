...
class TestRecordDirect:
    ...
    def test_should_direct_list_record(self):
        setup = _record_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-record", "live" if setup["live"] else "unit")
        if _skip:
...
def _record_direct_setup(mockres):
