...
from neugeborenenvornamenkantonstgallen_sdk.core.response import NeugeborenenVornamenKantonStgallenResponse
from neugeborenenvornamenkantonstgallen_sdk.core.operation import NeugeborenenVornamenKantonStgallenOperation
...
def _make_ctx_from_map(ctxmap, client, utility):
    ...
    if isinstance(spec_map, dict):
        ctx.spec = NeugeborenenVornamenKantonStgallenSpec(spec_map)

    # Handle result from JSON map
    res_map = ctxmap.get("result")
    if isinstance(res_map, dict):
        ctx.result = NeugeborenenVornamenKantonStgallenResult(res_map)
        err_map = res_map.get("err")
        if isinstance(err_map, dict):
            msg = err_map.get("message", "")
            ctx.result.err = NeugeborenenVornamenKantonStgallenError("", msg)

    # Handle response from JSON map
    resp_map = ctxmap.get("response")
    if isinstance(resp_map, dict):
        ctx.response = NeugeborenenVornamenKantonStgallenResponse(resp_map)
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return NeugeborenenVornamenKantonStgallenError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_done_basic(self):
        ...
        def subject(entry):
            ...
            except NeugeborenenVornamenKantonStgallenError as e:
                return None, e
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = NeugeborenenVornamenKantonStgallenResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_add_basic(self):
        ...
        feature = NeugeborenenVornamenKantonStgallenBaseFeature()
        utility.feature_add(ctx, feature)

        assert len(client.features) == start_len + 1

    def test_feature_hook_basic(self):
        hook_client = NeugeborenenVornamenKantonStgallenSDK.test(None, None)
        hook_utility = hook_client._utility
        ctx = _make_test_ctx(hook_client, hook_utility)

        state = {"called": False}

        class TestHookFeature(NeugeborenenVornamenKantonStgallenBaseFeature):
    ...
    def test_feature_init_basic(self):
        init_client = NeugeborenenVornamenKantonStgallenSDK.test(None, None)
        ...
        class TestInitFeature(NeugeborenenVornamenKantonStgallenBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(NeugeborenenVornamenKantonStgallenBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_live(self):
        ...
        live_client = NeugeborenenVornamenKantonStgallenSDK({
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = NeugeborenenVornamenKantonStgallenSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_fetch_def_basic(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = NeugeborenenVornamenKantonStgallenSpec({
            "base": "http://localhost:8080",
            "prefix": "/api",
            "path": "items/{id}",
            "suffix": "",
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = NeugeborenenVornamenKantonStgallenSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = NeugeborenenVornamenKantonStgallenSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = NeugeborenenVornamenKantonStgallenOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = NeugeborenenVornamenKantonStgallenSDK.test(None, setup_opts)
