...
import (
	...
	omni "github.com/voxgig-sdk/neugeborenen-vornamen-kanton-stgallen-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NeugeborenenVornamenKantonStgallenError); is {
				out := map[string]any{
					"name":    "NeugeborenenVornamenKantonStgallenError",
...
func omniCtx(arg any, client *sdk.NeugeborenenVornamenKantonStgallenSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NeugeborenenVornamenKantonStgallenSDK); is {
			client = live
