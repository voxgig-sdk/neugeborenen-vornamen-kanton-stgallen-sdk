...
class RecordEntity
  ...
  def make
    opts = @_entopts.dup
    RecordEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = NeugeborenenVornamenKantonStgallenHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Record, Hash] the current Record data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = NeugeborenenVornamenKantonStgallenHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Record fields)
  def match_get
  ...
  #
  # @param reqmatch [RecordListMatch, Hash, nil] match filter (any subset of
  #   Record fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Record>, Array] the matching Record items; raises NeugeborenenVornamenKantonStgallenError on failure
