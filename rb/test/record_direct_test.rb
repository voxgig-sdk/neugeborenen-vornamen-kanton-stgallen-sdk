...
class RecordDirectTest < Minitest::Test
  def test_direct_list_record
    setup = record_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-record", setup[:live] ? "live" : "unit")
    if _should_skip
...
def record_direct_setup(mockres)
  ...
  if live
    ...
    client = NeugeborenenVornamenKantonStgallenSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
