...
module NeugeborenenVornamenKantonStgallenUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NeugeborenenVornamenKantonStgallenResponse.new({})
    ...
    if fetch_err
      ...
      response = NeugeborenenVornamenKantonStgallenResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NeugeborenenVornamenKantonStgallenResponse.new(fetched)
    else
